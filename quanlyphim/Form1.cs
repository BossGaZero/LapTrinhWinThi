...
namespace quanlyphim
{
    public partial class Form1 : Form
    {
        ...
        static string sqlconnect = @"Data Source=123-PC\SQLEXPRESS;Initial Catalog=QLPhim;Integrated Security=True";

        private void Form1_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(sqlconnect);
            con.Open();
            String sql = "SELECT MaPhim, TenPhim, TheLoai, DinhDang3D, ThoiLuong, MoTa , TrangThai FROM Phim";
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand command = new SqlCommand();
            command = con.CreateCommand();
            command.CommandText = "UPDATE Phim  SET Trangthai=0  WHERE MaPhim = '" + maphim + "'";
            int i = command.ExecuteNonQuery();
            if (i > 0)
            {
